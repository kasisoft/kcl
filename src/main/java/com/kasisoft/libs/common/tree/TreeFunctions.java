...
public class TreeFunctions {
    ...
    private static List<String> split(@NotBlank String str) {
        return Arrays.asList(str.split("/")).stream().map(StringFunctions::cleanup).filter(Predicates.notNull()).collect(Collectors.toCollection(LinkedList::new));
    }
    ...
    public static <T> NamedTreeNode<T> parenthesize(@NotNull List<T> values, @NotNull Function<T, String> toPath, TriConsumer<String, T, T> addChild) {
        ...
        values.stream().map($ -> new NamedTreeNode($, split(toPath.apply($)))).forEach(nodes::add);

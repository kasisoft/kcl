...
public class FileAdapterTest {
    ...
    public static Stream<Arguments> data_decode() {
        ...
        return Stream.of(Arguments.of(null, null), Arguments.of("%s\\http.xsd".formatted(path), TEST_RESOURCES.getResourceAsFile("http.xsd")), Arguments.of("%s/http.xsd".formatted(path), TEST_RESOURCES.getResourceAsFile("http.xsd")), Arguments.of("%s\\bibo.txt".formatted(path), TEST_RESOURCES.findResourceAsFile("bibo.txt").orElse(null)), Arguments.of("%s/bibo.txt".formatted(path), TEST_RESOURCES.findResourceAsFile("bibo.txt").orElse(null)));
    }
    ...
    public static Stream<Arguments> data_encode() {
        var path = TEST_RESOURCES.getRootFolder().toString();
        return Stream.of(Arguments.of(null, null), Arguments.of(TEST_RESOURCES.getResourceAsFile("http.xsd"), "%s/http.xsd".formatted(path)), Arguments.of(TEST_RESOURCES.getResourceAsFile("bibo.txt"), "%s/bibo.txt".formatted(path)));
    }
